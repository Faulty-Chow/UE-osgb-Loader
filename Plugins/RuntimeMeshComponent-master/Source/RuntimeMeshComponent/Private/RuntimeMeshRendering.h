...
#if ENGINE_MAJOR_VERSION == 5
		FRHIResourceCreateInfo CreateInfo(TEXT("FRuntimeMeshVertexBuffer::CreateRHIBuffer"), &InStream);
#else
...
public:
	...
	{
#if ENGINE_MAJOR_VERSION == 5
		EBufferUsageFlags Flags = (bDynamicBuffer ? BUF_Dynamic : BUF_Static) | BUF_ShaderResource;
#else
		const int32 Flags = (bDynamicBuffer ? BUF_Dynamic : BUF_Static) | BUF_ShaderResource;
#endif
		CreateInfo.bWithoutNativeResource = SizeInBytes <= 0;
...
#if ENGINE_MAJOR_VERSION == 5
		FRHIResourceCreateInfo CreateInfo(TEXT("FRuntimeMeshIndexBuffer::CreateRHIBuffer"), &InStream);
#else
		FRHIResourceCreateInfo CreateInfo(&InStream);
#endif
	...
	void InitRHIFromExisting(const FIndexBufferRHIRef& InIndexBufferRHI, int32 NumElements, bool bShouldUseHighPrecision)
	{
		...
		{
#if ENGINE_MAJOR_VERSION == 5
			FRHIResourceCreateInfo CreateInfo(TEXT("FRuntimeMeshIndexBuffer::InitRHIFromExisting"));
#else
